...
static long core(const bitvec_t *useless, int i, const enumeration_info_t *info)
{
  if(info->callback)
    info->callback(useless, i, info);
  return 1;
...
static void generate_slim_fat(doublequotient_t *dq, int *is_slim, int *is_fat)
{
  queue_t queue;
  int current;
  doublecoset_list_t *edge;
...
long enumerate_principal_balanced_thickenings(doublequotient_t *dq, enumeration_callback callback, void *callback_data)
{
  ...
  generate_slim_fat(dq, info.principal_is_slim, info.principal_is_fat);
...
long enumerate_core(doublequotient_t *dq, enumeration_callback callback, void *callback_data)
{
  ...
  info.principal_is_slim = (int*)malloc(info.size*sizeof(int));
  info.principal_is_fat = (int*)malloc(info.size*sizeof(int));
  // info.in_core = (int*)malloc(info.size*sizeof(int));
  ...
  bitvec_t pos;
  for (int i=0; i<info.size; i++){
    if (info.principal_is_slim[i] && !info.principal_is_slim[info.size-i-1]){
      count += core(&pos, i, &info);
    }
  }
